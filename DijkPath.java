...
public class DijkPath implements Path {
  ...
  Stack<BStop> path = new Stack<BStop>();
  ...
  public void computePath(String from, String to) {
    ...
    while(!pq.isEmpty()){
      ...
      for(Edge e : current.getValue().getEdges()){
        ...
        /* Om den nya längden är kortare uppdaterar vi nodens nyckel i pq:n. */
      ...
      do {
        path.push(current.getValue().getStop());
        current = current.getPrevious();
      } while (current != null);
      
      for(BStop bb : this.path){
